...
class User:
    ...
    def set_options_from_profile(self):
        ...
        empty_info_string_list = [dict.parameters_dict[x] for x in empty_info_list]
        if len(empty_info_list) > 0:
            ...
            message_user_info = f"Информация вашего профиля:\nВозраст: {self.age}\nпол: {dict.sex_dict[self.sex]}\nгород: {self.city['title']}\nсемейное положение: {dict.relations_dict[self.sex][self.relation]}."
