...
def interaction_with_user(group_bot_vk, user, user_id):
    ...
    while True:
        user_answer = group_bot_vk.get_info_from_user(user_id=user_id,
                                                      message=dict.options_messages['start'],
                                                      answers_list=['1', '2', '3'])
        if user_answer == '1':
            ...
            group_bot_vk.write_msg(user_id=user_id, message=dict.options_messages['end_work'])
            return False
...
def main():
    ...
    """
    current_users = {}
    group_bot_vk = flirt_bot.BotVK(private_token.USER_TOKEN, private_token.GROUP_TOKEN)
    while True:
